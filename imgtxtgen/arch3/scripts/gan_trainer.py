...
from imgtxtgen.arch3.utils.config import getConfigFromDict, getDictFromConfig, BaseConfig
from imgtxtgen.arch3.utils.image_transform import NumpyResize, NumpyToTensor
...
class GANTrainer:
    r"""
    A class managing a progressive GAN training. Logs, chekpoints, visualization, and number iterations are managed here.
    """
    ...
    def __init__(self,
        ...
        if (not ignoreAttribs) and (self.pathAttribDict is not None or self.imagefolderDataset):
            self.modelConfig.attribKeysOrder = self.getDataset(0, size=10).getKeyOrders()
        ...
        if self.checkPointDir is not None:
            self.pathLossLog = os.path.abspath(os.path.join(self.checkPointDir, self.modelLabel + '_losses.pkl'))
            self.pathRefVector = os.path.abspath(os.path.join(self.checkPointDir, self.modelLabel + '_refVectors.pt'))
    ...
    def updateLossProfile(self, iterator):

        nPrevIter = len(self.lossProfile[-1]["iter"])
        self.lossProfile[-1]["iter"].append(iterator)
    ...
    def loadSavedTraining(self, pathModel, pathTrainConfig, pathTmpConfig, loadGOnly=False, loadDOnly=False, finetune=False):
        ...
        Args:
            ...
            - pathModel (string): path to the file containing the model structure (.pt)
        ...
        if tmpPathLossLog is None:
            self.lossProfile = [{"iter": [], "scale": self.startScale}]
        elif not os.path.isfile(tmpPathLossLog):
            print("WARNING : couldn't find the loss logs at " + tmpPathLossLog + " resetting the losses")
            self.lossProfile = [{"iter": [], "scale": self.startScale}]
        else:
            ...
            if self.lossProfile[-1]["iter"][-1] > self.startIter:
                ...
                for item in self.lossProfile[-1]:
                    if isinstance(self.lossProfile[-1][item], list):
                        self.lossProfile[-1][item] = self.lossProfile[-1][item][:indexStop]
        ...
        self.initModel()
        self.model.load(pathModel, loadG=not loadDOnly, loadD=not loadGOnly, finetuning=finetune)
    ...
    def saveCheckpoint(self, outDir, outLabel, scale, iterator):
        r"""
        ...
        if self.visualisation is not None:
            ...
            self.visualisation.saveTensor(ref_g, (imgSize, imgSize), os.path.join(outDir, outLabel + '.jpg'))
    ...
    def sendToVisualization(self, refVectorReal, scale, label=None):
        ...
        if label is None:
            ...
            self.visualisation.publishTensors(refVectorReal, (imgSize, imgSize), label + " real", env=envLabel)

        ref_g_smooth = self.model.test(self.refVectorVisualization, True)
        self.tokenWindowFakeSmooth = self.visualisation.publishTensors(ref_g_smooth, (imgSize, imgSize), label + " smooth", self.tokenWindowFakeSmooth, env=envLabel)

        ref_g = self.model.test(self.refVectorVisualization, False)

        self.tokenWindowFake = self.visualisation.publishTensors(ref_g, (imgSize, imgSize), label + " fake", self.tokenWindowFake, env=envLabel)
        self.tokenWindowReal = self.visualisation.publishTensors(refVectorReal, (imgSize, imgSize), label + " real", self.tokenWindowReal, env=envLabel)
        self.tokenWindowLosses = self.visualisation.publishLoss(self.lossProfile[scale], self.modelLabel, self.tokenWindowLosses, env=envLabel)
    ...
    def getDBLoader(self, scale):
        ...
        return torch.utils.data.DataLoader(dataset, batch_size=self.modelConfig.miniBatchSize, shuffle=True, num_workers=self.model.n_devices)
    ...
    def inScaleUpdate(self, iterator, scale, inputs_real):
        return inputs_real
