...
from imgtxtgen.arch3.utils.standard_configurations.pgan_config import _C
from imgtxtgen.arch3.models.progressive_gan import ProgressiveGAN
from imgtxtgen.arch3.scripts.gan_trainer import GANTrainer
from imgtxtgen.arch3.utils.utils import getMinOccurence
...
class ProgressiveGANTrainer(GANTrainer):
    ...
    def __init__(self, pathdb, miniBatchScheduler=None, datasetProfile=None, configScheduler=None, **kwargs):
        r"""
        Args:
            - pathdb (string): path to the directorty containing the image dataset
            - useGPU (bool): set to True if you want to use the available GPUs for the training procedure
            - visualisation (module): if not None, a visualisation module to follow the evolution of the training
            - lossIterEvaluation (int): size of the interval on which the model'sloss will be evaluated
            - saveIter (int): frequency at which at checkpoint should be saved (relevant only if modelLabel != None)
            - checkPointDir (string): if not None, directory where the checkpoints should be saved
            - modelLabel (string): name of the model
            - config (dictionary): configuration dictionnary. See std_p_gan_config.py for all the possible options
            - numWorkers (int): number of GOU to use. Will be set to one if not useGPU
            - stopOnShitStorm (bool): should we stop the training if a diverging behavior is detected ?
        """
    ...
    def updateAlphaJumps(self, nJumpScale, sizeJumpScale):
        ...
        Args:
            ...
            - nJumpScale (list of int): for each scale, the number of times alpha should be updated
            - sizeJumpScale (list of int): for each scale, the number of iterations between two updates
        """
    ...
    def inScaleUpdate(self, iterator, scale, input_real):

        if self.indexJumpAlpha < len(self.modelConfig.iterAlphaJump[scale]):
            if iterator == self.modelConfig.iterAlphaJump[scale][self.indexJumpAlpha]:
    ...
    def train(self):
        ...
        for scale in range(self.startScale, n_scales):
            ...
            while scale >= len(self.lossProfile):
                self.lossProfile.append({"scale": scale, "iter": []})
            ...
            while shiftIter < self.modelConfig.maxIterAtScale[scale]:
                ...
                status = self.trainOnEpoch(dbLoader, scale, shiftIter=shiftIter, maxIter=self.modelConfig.maxIterAtScale[scale])
            ...
            if self.checkPointDir is not None:
                ...
                label = self.modelLabel + ("_s%d_i%d" % (scale, realIter))
                self.saveCheckpoint(self.checkPointDir, label, scale, realIter)
            if scale == n_scales - 1:
                break
